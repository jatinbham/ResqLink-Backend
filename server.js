...
// ── N8n incoming webhook receiver ────────────────────────────────
// N8n can POST back to this endpoint with workflow results/updates
app.post("/api/n8n/callback", express.json(), (req, res) => {
  const { event, requestId, volunteerId, status, data } = req.body;
  console.log("[N8n Callback]", event, { requestId, volunteerId, status });

  // Add your callback handling logic here (e.g., update DB, send socket event)
  // This is where n8n will POST back after completing a workflow
